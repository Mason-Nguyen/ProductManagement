...
const ApproverDashboard: React.FC = () => {
    ...
    // Charts state
    const [requestsChartData, setRequestsChartData] = useState<any>(null);
    ...
    const fetchDashboardData = useCallback(async () => {
        try {
            setIsLoading(true);
            const [productsResult, pendingResult, approvedResult, rejectedResult] = 
            ...
            if (pendingResult.status === 'fulfilled') {
                setPendingRequests(pendingResult.value.length);
            } else {
                console.error('Failed to fetch pending requests:', pendingResult.reason);
                setPendingRequests(0);
            }

            // Handle approved requests count
            if (approvedResult.status === 'fulfilled') {
                setApprovedRequests(approvedResult.value.length);
            } else {
                console.error('Failed to fetch approved requests:', approvedResult.reason);
                setApprovedRequests(0);
            }

            // Handle rejected requests count
            if (rejectedResult.status === 'fulfilled') {
                setRejectedRequests(rejectedResult.value.length);
            } else {
                console.error('Failed to fetch rejected requests:', rejectedResult.reason);
                setRejectedRequests(0);
            }

            // Process requests chart data (last 3 months) - ALL REQUESTS
            const allApproverRequests = [
                ...(pendingResult.status === 'fulfilled' ? pendingResult.value : []),
                ...(approvedResult.status === 'fulfilled' ? approvedResult.value : []),
                ...(rejectedResult.status === 'fulfilled' ? rejectedResult.value : [])
            ];

            if (allApproverRequests.length > 0) {
                const last3Months = getLast3Months();
                const monthlyRequestsData = last3Months.map(monthInfo => {
                    const monthRequests = allApproverRequests.filter(req => {
                ...
                setRequestsChartData({
                    labels: monthlyRequestsData.map(d => d.label),
                    datasets: [
                        {
                            type: 'line' as const,
                            label: t('dashboard.expectedTotalPrice'),
                            data: monthlyRequestsData.map(d => d.expectedTotal),
                            borderColor: '#8b5cf6',
                        ...
                        },
                        {
                            ...
                            data: monthlyRequestsData.map(d => d.actualTotal),
            ...
            <div className="dashboard-content fade-in">
                ...
                <div className="stats-grid">
                    ...
                    <div className="stat-card">
                        ...
                        <div className="stat-label">{t('stat.approvedRequests')}</div>
                    </div>
                    <div className="stat-card">
                        ...
                        <div className="stat-label">{t('stat.rejectedRequests')}</div>
                    </div>
                </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginTop: '1.5rem' }}>
                            ...
                            <div className="content-card">
                                ...
                                <div style={{ height: '300px' }}>
                                    {requestsChartData ? (
                                        <Chart type="bar" data={requestsChartData} options={mixedChartOptions} />
                                    ) : (
